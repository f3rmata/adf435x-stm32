...
// defined in main.c, note external linkage.
extern SPI_HandleTypeDef hspi1;

struct adf4350_platform_data platdata;
...
void SPI_assert_CS_LD() {
  HAL_GPIO_WritePin(ADF4351_CE_GPIO_Port, ADF4351_CE_Pin, 0);
  HAL_GPIO_WritePin(ADF4351_LE_GPIO_Port, ADF4351_LE_Pin, 0);
}
...
void SPI_deassert_CS_LD() {
  HAL_GPIO_WritePin(ADF4351_CE_GPIO_Port, ADF4351_CE_Pin, 1);
  HAL_GPIO_WritePin(ADF4351_LE_GPIO_Port, ADF4351_LE_Pin, 1);
}

/************************
 *  @brief write 8 bits of data on SPI interface MOSI pin..
 */
void SPI_Write(uint8_t data) { HAL_SPI_Transmit(&hspi1, &data, 1, 1000); }
...
int32_t adf4350_sync_config(struct adf4350_state *st) {
  ...
  for (i = ADF4350_REG5; i >= ADF4350_REG0; i--) {
    if ((st->regs_hw[i] != st->regs[i]) || ((i == ADF4350_REG0) && doublebuf)) {
      switch (i) {
      case ADF4350_REG1:
      case ADF4350_REG4:
        doublebuf = 1;
...
int64_t adf4350_set_freq(struct adf4350_state *st, uint64_t freq) {
  ...
  band_sel_div =
      st->fpfd % ADF4350_MAX_BANDSEL_CLK > ADF4350_MAX_BANDSEL_CLK / 2
  ...
  st->regs[ADF4350_REG0] =
      ADF4350_REG0_INT(st->r0_int) | ADF4350_REG0_FRACT(st->r0_fract);

  st->regs[ADF4350_REG1] =
      ADF4350_REG1_PHASE(1) | ADF4350_REG1_MOD(st->r1_mod) | prescaler;

  st->regs[ADF4350_REG2] =
      ADF4350_REG2_10BIT_R_CNT(r_cnt) | ADF4350_REG2_DOUBLE_BUFF_EN |
      (pdata->ref_doubler_en ? ADF4350_REG2_RMULT2_EN : 0) |
      (pdata->ref_div2_en ? ADF4350_REG2_RDIV2_EN : 0) |
      (pdata->r2_user_settings &
       (ADF4350_REG2_PD_POLARITY_POS | ADF4350_REG2_LDP_6ns |
        ADF4350_REG2_LDF_INT_N | ADF4350_REG2_CHARGE_PUMP_CURR_uA(5000) |
        ADF4350_REG2_MUXOUT(0x7) | ADF4350_REG2_NOISE_MODE(0x9)));

  st->regs[ADF4350_REG3] =
      pdata->r3_user_settings &
      (ADF4350_REG3_12BIT_CLKDIV(0xFFF) | ADF4350_REG3_12BIT_CLKDIV_MODE(0x3) |
       ADF4350_REG3_12BIT_CSR_EN);

  st->regs[ADF4350_REG4] =
      ADF4350_REG4_FEEDBACK_FUND | ADF4350_REG4_RF_DIV_SEL(st->r4_rf_div_sel) |
      ADF4350_REG4_8BIT_BAND_SEL_CLKDIV(band_sel_div) | ADF4350_REG4_RF_OUT_EN |
      (pdata->r4_user_settings &
       (ADF4350_REG4_OUTPUT_PWR(0x3) | ADF4350_REG4_AUX_OUTPUT_PWR(0x3) |
        ADF4350_REG4_AUX_OUTPUT_EN | ADF4350_REG4_AUX_OUTPUT_FUND |
  ...
  tmp =
      (uint64_t)((st->r0_int * st->r1_mod) + st->r0_fract) * (uint64_t)st->fpfd;
...
int32_t adf4350_setup(adf4350_init_param init_param) {
  ...
  st->pdata->r2_user_settings =
      init_param.phase_detector_polarity_positive_enable
          ...
          : 0;
  st->pdata->r2_user_settings |=
      init_param.lock_detect_precision_6ns_enable ? ADF4350_REG2_LDP_6ns : 0;
  st->pdata->r2_user_settings |=
      init_param.lock_detect_function_integer_n_enable ? ADF4350_REG2_LDF_INT_N
                                                       : 0;
  st->pdata->r2_user_settings |=
  ...
  st->pdata->r3_user_settings =
      init_param.cycle_slip_reduction_enable ? ADF4350_REG3_12BIT_CSR_EN : 0;
  ...
  st->pdata->r4_user_settings =
      init_param.aux_output_enable ? ADF4350_REG4_AUX_OUTPUT_EN : 0;
  st->pdata->r4_user_settings |= init_param.aux_output_fundamental_enable
                                     ? ADF4350_REG4_AUX_OUTPUT_FUND
                                     : 0;
  st->pdata->r4_user_settings |=
      init_param.mute_till_lock_enable ? ADF4350_REG4_MUTE_TILL_LOCK_EN : 0;
  st->pdata->r4_user_settings |=
